...
func TestTopUpQuotaValidation(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			amount:      math.MaxInt64 / 500000,
			wantQuota:   (math.MaxInt64 / 500000) * 500000,
		},
		{
			name:        "currency amount above limit",
			displayType: operation_setting.QuotaDisplayTypeUSD,
			amount:      math.MaxInt64/500000 + 1,
			wantErr:     true,
		},
		{
		...
		{
			name:        "token amount must be positive",
			displayType: operation_setting.QuotaDisplayTypeTokens,
			amount:      0,
			wantErr:     true,
		},
...
func TestValidateTopUpQuotaReturnsMaximumAmount(t *testing.T) {
	...
	maxAmount := decimal.NewFromInt(math.MaxInt64).
...
func TestRequestAmountRejectsTopUpThatCannotBeSettled(t *testing.T) {
	...
	ctx.Request = httptest.NewRequest(
		http.MethodPost,
		"/api/user/amount",
		strings.NewReader(fmt.Sprintf(`{"amount":%d}`, math.MaxInt64/500000+1)),
	)
	...
	assert.JSONEq(t, fmt.Sprintf(`{"message":"error","data":"单笔充值数量不能大于 %d"}`, math.MaxInt64/500000), recorder.Body.String())
}
...
func TestRequestAmountRejectsTopUpThatWouldOverflowWallet(t *testing.T) {
	...
	ctx.Request = httptest.NewRequest(
		http.MethodPost,
		"/api/user/amount",
		strings.NewReader(fmt.Sprintf(`{"amount":%d}`, math.MaxInt64/500000)),
	)
	ctx.Request.Header.Set("Content-Type", "application/json")
...
func TestValidateCreditedQuotaRejectsOverflow(t *testing.T) {
	_, err := validateCreditedQuota(decimal.NewFromInt(math.MaxInt64))
	require.NoError(t, err)
	_, err = validateCreditedQuota(decimal.Zero)
	require.EqualError(t, err, "充值额度必须大于 0")
	_, err = validateCreditedQuota(decimal.NewFromInt(math.MaxInt64).Add(decimal.NewFromInt(1)))
	require.EqualError(
		t,
		err,
		"充值额度超出系统可表示范围",
...
func TestStripeCreditedQuotaIncludesGroupRatio(t *testing.T) {
	...
	maxAmount := int64(math.MaxInt64 / 1_000_000)
	_, err := validateCreditedQuota(getStripeCreditedQuota(maxAmount, "vip"))
	require.NoError(t, err)
	_, err = validateCreditedQuota(getStripeCreditedQuota(maxAmount+1, "vip"))
	require.Error(t, err)
