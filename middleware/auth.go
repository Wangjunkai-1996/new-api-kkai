...
// VideoStudioMediaAuth keeps the historical route name while using the same
// stateless dashboard access token contract as every other studio endpoint.
...
func TokenAuth() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		if len(token.GetIpLimits()) > 0 {
			...
			if err := model.ValidateUserTokenIP(token, clientIp); errors.Is(err, model.ErrTokenClientIPInvalid) {
