...
`duration` 和 `seconds` 都可以省略。缺失、`null`、空字符串、`0` 或 `1-3` 会由本站按 `4`
秒处理并按 `4` 秒计费；负数、小数、无法解析的值和超过对应模型上限的值会返回 400。
两者同时发送时，`null`、空字符串和 `0` 视为未提供，由另一个字段接管；两个非空且
非 `0` 的值会先各自按最少 4 秒规范化，结果必须相等。客户端仍建议显式发送 `4-15`
（2.0）或 `4-30`（2.5）范围内的整数，例如 `5` 或 `"5"`；不要发送 `5.0` 或 `"05"`。
