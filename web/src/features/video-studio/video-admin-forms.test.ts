...
function normalizeJSONValue(value: unknown): unknown {
  // oxlint-disable-next-line unicorn/prefer-structured-clone -- JSON normalization intentionally removes undefined properties.
...
describe('video model admin form', () => {
  ...
  test('keeps an unchanged specification version and bumps a changed one', () => {
    ...
    assert.deepEqual(
      normalizeJSONValue(unchanged.specification),
      normalizeJSONValue(profile.specification)
    )
    ...
    assert.deepEqual(
      normalizeJSONValue(changedProfileDefault.specification),
      normalizeJSONValue(profile.specification)
    )
