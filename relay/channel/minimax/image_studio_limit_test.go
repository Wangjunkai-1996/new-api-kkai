...
func TestMiniMaxImageStudioResponseLimitRejectsOversizedBody(t *testing.T) {
	c := limitedMiniMaxImageContext(32)
	response := &http.Response{
		StatusCode: http.StatusOK, Header: make(http.Header),
		Body: io.NopCloser(bytes.NewReader(bytes.Repeat([]byte("x"), 33))),
	}
	_, apiErr := miniMaxImageHandler(c, response, &relaycommon.RelayInfo{})
	require.NotNil(t, apiErr)
	require.Equal(t, service.ErrImageStudioResponseTooLarge.Error(), apiErr.Error())
}

func limitedMiniMaxImageContext(maximum int64) *gin.Context {
