...
func (a *TaskAdaptor) DoResponse(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) (*channel.TaskSubmitResponse, *channel.TaskResponseError) {
	...
	if strings.TrimSpace(s.Name) == "" {
		return nil, channel.NewUncertainTaskResponseError(service.TaskErrorWrapper(fmt.Errorf("missing operation name"), "invalid_response", http.StatusInternalServerError))
	}
	...
	buffered, err := channel.NewJSONTaskSubmitResponse(localID, responseBody, ov)
	if err != nil {
