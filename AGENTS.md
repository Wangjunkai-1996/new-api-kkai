...
### Fork-Only Production Patches — MUST Preserve

This fork carries local production patches for the KKAI/Kkrich deployment. Official upstream images do not include these behaviors, so do not remove or overwrite them during upstream syncs.

**FRT header display patch:**

- `RelayInfo` must keep `UpstreamHeaderTime time.Time`.
- `relay/channel/api_request.go` must record the upstream HTTP response header timestamp immediately after a successful `client.Do(req)` response.
- `service.GenerateTextOtherInfo` must write displayed `other.frt` from `UpstreamHeaderTime - StartTime` when available.
- The original first SSE timing must remain available as `other.first_sse_ms`.
- Before building or deploying an upstream sync, run `scripts/check-frt-header-patch.sh`.
...
- High-confidence policy incidents such as `cyber_policy` must stop retry fan-out.
- Classify causality before client-token action:
  - `client_policy_request`: token breaker and optional persistent token/user disable are allowed.
  - `upstream_key_encountered`: never set the client-token breaker and never persistently disable the client token/user.
- The implicated upstream channel/key must not be isolated by default. Only isolate it through the existing channel status and breaker mechanisms when `policy_incident_setting.isolate_upstream_on_policy_incident` is explicitly enabled.
- `policy_incident_events` must remain append-only and store only redacted metadata plus upstream key fingerprints.
- Normal relay and task relay paths must both call the policy-incident handling flow.
- Before build/deploy, run the focused policy tests documented in `docs/ops/policy-incident-patch.md`.

**CC Switch one-click import patch:**

- CC Switch one-click import must default provider names to `KKAI`.
- The imported provider endpoint must be `https://api.kkrich.ltd/v1`.
- Import links must include usage-query parameters backed by `/api/usage/token/` and the imported token key.
- `/api/usage/token/` must expose user-balance fields (`user_total_available`, `user_total_used`, `user_total_granted`) for CC Switch balance display.
- Preserve this behavior in both frontend variants:
