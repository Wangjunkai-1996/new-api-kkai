...
| `duration` | integer 或整数字符串 | 至少一个* | 4-15 | 4-30 | 推荐字段。必须是整数秒。 |
| `seconds` | integer 或整数字符串 | 至少一个* | 4-15 | 4-30 | 兼容字段；2.0 示例常用字符串。与 `duration` 同时传时数值必须相同。 |
...
`*` Seedance 特价适配器要求显式提供 `duration` 或 `seconds` 至少一个；公共网关对其他
视频适配器可能在缺省时按 4 秒估算，但不要依赖这个兼容行为。两者同时传时请由客户端
保证整数值相同；当前计费估算会优先读取 `seconds`。请传规范整数或规范整数字符串，
例如 `5` 或 `"5"`，不要传 `5.0`、`"05"`、负数或小数。
...
| HTTP | code / 特征 | 处理 |
| --- | --- | --- |
| 400 | `invalid_request`、`invalid_seconds`、`task_not_exist`，或适配器参数校验失败（实际 `code/message` 以返回为准）；下载时任务未完成/Range 无效 | 修正字段或确认任务 ID；不要盲目重发未修改的 POST，只有明确修正参数后再由业务决定是否新建任务。 |
...
2. `duration`/`seconds` 至少显式传入一个，并且是 4-15（2.0）或 4-30（2.5）的整数；两者同传时数值相等。
